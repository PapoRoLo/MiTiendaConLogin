...
namespace MiTiendaConLogin.Areas.Identity.Pages.Account.Manage
{
    public class EmailModel : PageModel
    {
        ...
        public class InputModel
        {
            [Required]
            [EmailAddress]
            [Display(Name = "Nuevo email")] // <-- Traducido
            public string NewEmail { get; set; }
        }

        private async Task LoadAsync(ApplicationUser user) // <-- Corregido
        {
        ...
        public async Task<IActionResult> OnPostChangeEmailAsync()
        {
            ...
            if (Input.NewEmail != email)
            {
                ...
                await _emailSender.SendEmailAsync(
                    Input.NewEmail,
                    "Confirma tu email", // <-- Traducido
                    $"Por favor confirma tu cuenta <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>haciendo clic aquí</a>."); // <-- Traducido

                StatusMessage = "Enlace de confirmación para cambiar email enviado. Por favor, revisa tu correo."; // <-- Traducido
                return RedirectToPage();
            }

            StatusMessage = "Tu email no ha cambiado."; // <-- Traducido
            return RedirectToPage();
        }
        ...
        public async Task<IActionResult> OnPostSendVerificationEmailAsync()
        {
            ...
            StatusMessage = "Email de verificación enviado. Por favor, revisa tu correo."; // <-- Traducido
            return RedirectToPage();
        }
    }
}